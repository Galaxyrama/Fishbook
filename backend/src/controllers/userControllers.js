...
export const logIn = async(req, res, next) => {
    try {
        ...
        if(!password || !email) {
            return res.status(400).json({ message: "Please provide the correct email address and password!" });
        }
...
export const signUp = async(req, res, next) => {
    ...
    try {
        if(!username || !passwordRaw || !email) {
            return res.status(400).json({ message: "Please provide an username, email address and password!" });
        }
